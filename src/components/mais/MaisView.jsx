              ...
              <div className="space-y-1.5">
                {participantes.map((p, i) => (
                  <div key={i} className="flex items-center gap-2 text-[14px]">
                    <span className="w-7 h-7 rounded-full bg-blue/10 text-blue flex items-center justify-center text-[12px] font-bold flex-shrink-0">
                      {p.profiles?.nome?.[0]?.toUpperCase() ?? '?'}
                    </span>
                    <span className="flex-1 font-medium truncate">{p.profiles?.nome ?? 'Usuário'}</span>
                    {p.papel === 'owner' && (
                      <span className="text-[11px] font-semibold text-orange bg-orange/10 px-2 py-0.5 rounded-full flex items-center gap-1">
                        <Crown className="w-3 h-3" /> Owner
                      </span>
                    )}
                    {p.papel === 'editor' && (
                      <span className="text-[11px] font-semibold text-blue/70 bg-blue/5 px-2 py-0.5 rounded-full">Editor</span>
                    )}
                  </div>
                ))}
              </div>
